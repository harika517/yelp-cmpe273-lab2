...
router.post('/rest/addprofilepic/', auth, async(req, res) => {
    ...
    console.log('inside image upload, restuser id is,', req.restuser.id);
    ...
    await useruploads(req, res, async(err) => {
        ...
        if (!err) {
            try {
                ...
                const restuser = await RestUser.findOneAndUpdate({ restEmail: req.body.Email }, { image: req.file.filename }, { new: true });
                console.log('query result is', restuser.image);
