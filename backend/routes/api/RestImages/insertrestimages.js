...
const userstorage = multer.diskStorage({
    destination: (req, file, callback) => {
        ...
        const dir = `${path.join(__dirname, '../../../../')}/public/uploads/${req.params.Rest_Name.replace(' ', '_')}`;
...
router.post('/:Rest_Name', auth, async(req, res) => {
    await useruploads(req, res, async(err) => {
        ...
        if (!err) {
            ...
            try {
                ...
                const restprofile = await RestProfile.findOneAndUpdate({ restuser: objid }, { $push: { restimages: req.file.filename } }, { new: true });
                console.log('query result is', restprofile);
                if (!restprofile) {
